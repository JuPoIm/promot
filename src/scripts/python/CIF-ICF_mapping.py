...
columns = ['Foundation URI ICF', 'predicate_id', 'Code ICF', 'Label EN', 'URI data.esante','mapping_justification', 'author_id', 'mapping_date', 'confidence']
...
with open ('../scripts/python/data/CIF-ASIP.tsv', encoding='utf-8') as tsvfile:
        reader = csv.reader(tsvfile, delimiter = '\t', quotechar = '"', quoting = csv.QUOTE_MINIMAL)
        for row in reader:
...
sssom_df.to_csv('../scripts/python/data/ICF_to_CIF-ASIP_all.tsv', sep='\t', index=False)
